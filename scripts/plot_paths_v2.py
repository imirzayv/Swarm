...
def plot_targets_only(
    ...
    """
    Minimal targets-only figure: monitoring-area boundary plus the ground-truth
    targets (per-class colored X markers with their names). No title, no legend.
    """
    half = area_size / 2.0
    ...
    # draw_targets fills a throwaway list since we don't render a legend.
    draw_targets(ax, targets, [])
...
def plot_paths_with_detections(
    paths: dict[int, list[tuple[float, float]]],
    detections: list[tuple[float, float, str, int]],
    area_size: float,
    output_path: str,
    ...
    """
    Drone paths + per-class colored detection stars labeled "detection_<class>".
    Ground-truth targets are NOT drawn. No title, no legend.
    """
    half = area_size / 2.0
    ...
    for did in drone_ids:
        if did in paths:
            ...
            all_y.extend(pts[:, 1])
    if all_x:
        xmin = min(-half, min(all_x)) - 3
        xmax = max(half, max(all_x)) + 3
        ymin = min(-half, min(all_y)) - 3
        ymax = max(half, max(all_y)) + 3
    else:
        xmin, xmax = -half - 3, half + 3
        ymin, ymax = -half - 3, half + 3
    ...
    # Flight paths + start/end markers
    for did in drone_ids:
        if did not in paths or len(paths[did]) < 2:
            continue
        pts = np.array(paths[did])
        color = DRONE_COLORS.get(did, "#333333")
        ax.plot(pts[:, 0], pts[:, 1], color=color, linewidth=2.0, alpha=0.85, zorder=3)
        ax.plot(pts[0, 0], pts[0, 1], marker="^", color=color, markersize=14,
                markeredgecolor="black", markeredgewidth=1.2, zorder=5)
        ax.plot(pts[-1, 0], pts[-1, 1], marker="s", color=color, markersize=11,
                markeredgecolor="black", markeredgewidth=1.2, zorder=5)

    # Detections — every star drawn, but labels are placed once per cluster
    # so spatial detail is preserved without stacking 600+ texts on top of
    # each other. Cluster radius matches the detection→target match radius
    # used in metrics_compute (6 m).
    cluster_radius = 6.0
    by_class: dict[str, list[tuple[float, float]]] = {}
    for x, y, cls, did in detections:
    ...
    for cls, pts in by_class.items():
        ...
        # Greedy single-pass clustering by cluster_radius
        ...
        label = f"detection_{cls}" if cls else "detection"
        for cl in clusters:
            cx = sum(p[0] for p in cl) / len(cl)
            cy = sum(p[1] for p in cl) / len(cl)
            ax.annotate(
...
def main():
    ...
    # Targets-only plot (no title, no legend)
    targets_only_path = os.path.join(data_dir, "targets_only.png")
    plot_targets_only(targets, args.area_size, targets_only_path, dpi=args.dpi)

    # Paths + detections plot (no targets, no title, no legend)
    paths_detections_path = os.path.join(data_dir, "paths_detections.png")
    plot_paths_with_detections(
        paths, detections, args.area_size,
        paths_detections_path, drone_ids=drone_ids,
        dpi=args.dpi,
    )
